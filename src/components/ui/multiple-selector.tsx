...
interface MultipleSelectorProps {
  ...
  /** base key for react-query cache */
  queryKeyBase?: string;
...
const MultipleSelector = React.forwardRef<MultipleSelectorRef, MultipleSelectorProps>(
  (
    ...
    // Remote search with useQuery
    const {
      data: remoteOptions,
      isFetching: isFetchingRemote,
    } = useQuery({
      queryKey: [queryKeyBase, debouncedSearchTerm],
      queryFn: async () => {
        if (!onSearch) return [];
        const res = await onSearch(debouncedSearchTerm);
        return res || [];
      },
      enabled: !!onSearch && (triggerSearchOnFocus || debouncedSearchTerm.length > 0),
      staleTime,
    });

    // Unified data flow - single source of truth for options
    const options = React.useMemo<GroupOption>(() => {
      // Priority 1: Remote search (useQuery)
      if (onSearch && remoteOptions) {
        return transToGroupOption(remoteOptions, groupBy);
      }

      // Priority 2: Sync search
      if (onSearchSync) {
        if (triggerSearchOnFocus || debouncedSearchTerm) {
          const syncResults = onSearchSync(debouncedSearchTerm);
          return transToGroupOption(syncResults, groupBy);
        }
      }

      // Priority 3: Manual controlled options
      if (arrayOptions && !onSearch) {
        return transToGroupOption(arrayOptions, groupBy);
      }

      // Priority 4: Default options
